...
import type { UserProfile, UserSubscription, Plan } from '@/types'
...
function UserModal({ user, plans, onClose, onSaved }: {
  ...
  useEffect(() => {
    sb.from('user_subscriptions')
      ...
      .then(r => setSubs((r.data ?? []) as UserSubscription[]))
  }, [user.id])
  ...
  const addSubscription = async () => {
    ...
    const res = await sb.from('user_subscriptions').select('*, plan:plans(*)').eq('user_id', user.id).order('created_at', { ascending: false })
    setSubs((res.data ?? []) as UserSubscription[])
    setAddingSub(false)
  }

  const cancelSub = async (id: string) => {
    await sb.from('user_subscriptions').update({ status: 'cancelled' }).eq('id', id)
    ...
    <Modal title={`مدیریت: ${user.full_name || user.email}`} onClose={onClose} onSave={save} saving={saving}>
      ...
      <div className="flex items-center gap-3 mb-5 p-3 bg-ocean-900 rounded-xl">
        ...
        <div>
          ...
          <div className="text-slate-600 text-xs">عضو از {new Date(user.created_at).toLocaleDateString('fa-IR')}</div>
        </div>
      </div>
      ...
      <div className="mb-4">
              ...
              <div key={s.id} className="flex items-center justify-between p-3 bg-ocean-900 rounded-xl">
                <div>
                  <div className="flex items-center gap-2">
                    <span className="text-white text-sm font-medium">{(s.plan as any)?.name_fa}</span>
                    <span className={`text-xs px-2 py-0.5 rounded-full ${statusColor[s.status] ?? ''}`}>
                      {statusLabel[s.status]}
                    </span>
                  </div>
                  <div className="text-xs text-slate-500 mt-0.5">
                    {s.billing_cycle === 'monthly' ? 'ماهیانه' : 'سالیانه'} ·
                    انقضا: {new Date(s.expires_at).toLocaleDateString('fa-IR')}
                  </div>
                  <div className="text-xs text-slate-600">
                    {s.amount_paid.toLocaleString()} تومان
                  </div>
                </div>
                {s.status === 'active' && (
                  <button onClick={() => cancelSub(s.id)}
                    className="text-xs text-red-400 hover:text-red-300 px-2 py-1 bg-red-900/20 rounded-lg transition-colors">
                    لغو
                  </button>
                )}
              </div>
        ...
        <div className="p-3 border border-dashed border-ocean-500 rounded-xl">
          ...
          <div className="grid grid-cols-3 gap-2 mb-2">
            ...
            <input type="number" min="1" value={newSub.months}
              onChange={e => setNewSub(s => ({ ...s, months: e.target.value }))}
...
export default function UsersPage() {
  ...
  const toggleBlock = async (u: UserProfile) => {
    await sb.from('users').update({ is_blocked: !u.is_blocked, updated_at: new Date().toISOString() }).eq('id', u.id)
    load()
  }
        ...
        <div className="bg-ocean-800 border border-ocean-600 rounded-2xl overflow-hidden">
          ...
          <div className="flex items-center justify-between px-5 py-3 border-t border-ocean-600">
            <button onClick={() => setPage(p => Math.max(1, p - 1))} disabled={page === 1}
              className="text-xs text-slate-400 disabled:opacity-30 hover:text-white transition-colors px-3 py-1.5 border border-ocean-600 rounded-lg">
              ← قبلی
            </button>
