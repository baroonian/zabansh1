...
export default function CategoriesPage() {
  ...
  const openEdit = (c: Category) => {
    setForm({ name:c.name, name_fa:c.name_fa, description:c.description??'', color:c.color,
      icon:c.icon??'', sort_order:String(c.sort_order), is_active:c.is_active, image_url:(c as any).image_url??'' })
    setModal(c)
  }
  ...
  const save = async () => {
    ...
    setSaving(true)
    const payload = { name:form.name||form.name_fa, name_fa:form.name_fa, description:form.description||null,
      color:form.color, icon:form.icon||null, sort_order:parseInt(form.sort_order)||0,
  ...
  const del = async (id: string) => {
    ...
    await sb.from('categories').delete().eq('id', id); load()
  }
