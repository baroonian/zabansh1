...
function LessonModal({ lesson, chapterId, onClose, onSaved }: {
  ...
  const save = async () => {
    ...
    const res = lesson
      ? await sb.from('lessons').update(payload).eq('id', lesson.id)
...
function ChapterModal({ chapter, bookId, onClose, onSaved }: {
  ...
  const [f, setF] = useState({
    ...
    number: String(chapter?.number ?? 1),
  })
  ...
  useEffect(() => {
    if (chapter) {
      sb.from('lessons').select('*').eq('chapter_id', chapter.id).order('number')
  ...
  const save = async () => {
    ...
    const res = chapter
      ? await sb.from('chapters').update(payload).eq('id', chapter.id)
      : await sb.from('chapters').insert(payload)
    setSaving(false)
    if (res.error) { setErr(res.error.message); return }
    onSaved(); onClose()
  }
...
function BookModal({ book, cats, onClose, onSaved }: {
  ...
  const [f, setF] = useState({
    ...
    category_id: book?.category_id ?? (cats[0]?.id ?? ''),
  ...
  useEffect(() => {
    if (book) {
      sb.from('chapters').select('*').eq('book_id', book.id).order('number').then(r => setChapters(r.data ?? []))
    }
  }, [book?.id])
  ...
  const save = async () => {
    ...
    setSaving(true); setErr('')
    const payload = {
      title: f.title || f.title_fa,
      title_fa: f.title_fa,
      author: f.author || null,
      description: f.description || null,
      level: f.level as Book['level'],
    ...
    const res = book
      ...
      : await sb.from('books').insert({ ...payload, total_chapters: 0 })
    setSaving(false)
    if (res.error) { setErr(res.error.message); return }
    onSaved(); onClose()
  }
    ...
    <>
      <Modal title={book ? `ویرایش: ${book.title_fa}` : 'کتاب جدید'} onClose={onClose} onSave={save} saving={saving} wide>
        ...
        <label className="flex items-center gap-2 cursor-pointer mb-3">
          <div onClick={()=>setF(x=>({...x,is_active:!x.is_active}))}
            className={`w-10 h-5 rounded-full relative transition-colors ${f.is_active?'bg-amber-500':'bg-ocean-600'}`}>
            <div className={`absolute top-0.5 w-4 h-4 bg-white rounded-full transition-all ${f.is_active?'left-5':'left-0.5'}`} />
          </div>
              ...
              {chapters.map(ch => (
                <div key={ch.id} className="flex items-center justify-between px-3 py-2 bg-ocean-900 rounded-lg">
                  <span className="text-sm text-slate-200">
                    <span className="text-amber-500 ml-2">{ch.number}.</span>{ch.title_fa}
                  </span>
                  <div className="flex gap-1.5">
...
export default function BooksPage() {
  ...
  const LEVEL_BADGE: Record<string,string> = {
    ...
    advanced:'bg-amber-900/40 text-amber-400'
  }
            ...
            <tbody>
              {filtered.length === 0 ? (
                ...
                <tr key={b.id} className="border-b border-ocean-700 hover:bg-ocean-700 transition-colors">
                  ...
                  <td className="px-4 py-3.5">
                    <span className={`text-xs px-2 py-1 rounded-full ${LEVEL_BADGE[b.level]??''}`}>{LEVEL_LABEL[b.level]}</span>
                  </td>
                  <td className="px-4 py-3.5">
                  ...
                  <td className="px-4 py-3.5">
                    <div className="flex gap-2">
                      ...
                      <button onClick={()=>del(b.id)} className="text-xs text-red-400 px-2 py-1 bg-red-900/20 rounded-lg">🗑</button>
                    </div>
                  </td>
                </tr>
              ))}
