...
function LessonModal({ lesson, chapterId, onClose, onSaved }: { lesson?:Lesson; chapterId:string; onClose:()=>void; onSaved:()=>void }) {
  ...
  const save = async () => {
    ...
    if (lesson) {
      ...
      // درس جدید: بعد از insert نیاز داریم id را بگیریم تا دکمه رونویسی فعال شود
      const res = await sb.from('lessons').insert(payload).select('id').single()
      setSaving(false)
      if (res.error) { setErr(res.error.message); return }
      setSavedLessonId(res.data.id)
      onSaved()
      // مودال را نمی‌بندیم تا ادمین بتواند فوراً رونویسی خودکار را بزند
    }
  }
  ...
  return (
    <Modal title={lesson?'ویرایش درس':'درس جدید'} onClose={onClose} onSave={save} saving={saving} wide>
      ...
      {/* دکمه رونویسی خودکار با هوش مصنوعی */}
      <TranscribeButton
      ...
      {/* یا آپلود دستی فایل SRT برای همگام‌سازی دقیق‌تر */}
      <SrtUpload lessonId={savedLessonId} onSaved={() => setSrtCount(c => (c ?? 0) + 1)} />
...
// ── Book Modal ─────────────────────────────────────────────────
function BookModal({ book, cats, onClose, onSaved }: { book?:Book; cats:Category[]; onClose:()=>void; onSaved:()=>void }) {
  const sb = createClient()
  const [f, setF] = useState({
    ...
    is_active: book?.is_active??true, sort_order: String(book?.sort_order??0),
  })
  ...
  const save = async () => {
    ...
    const payload = {
      ...
      is_active: f.is_active, sort_order: parseInt(f.sort_order)||0,
    }
    const res = book ? await sb.from('books').update(payload).eq('id',book.id) : await sb.from('books').insert({...payload,total_chapters:0})
    setSaving(false)
    if (res.error) { setErr(res.error.message); return }
    onSaved(); onClose()
  }
  ...
  return (
    <div className="p-8">
      ...
      <div className="flex gap-3 mb-6 flex-wrap">
        <input value={search} onChange={e=>setSearch(e.target.value)} placeholder="جستجو..." className="px-4 py-2 bg-ocean-800 border border-ocean-600 rounded-xl text-white text-sm focus:outline-none focus:border-amber-500 w-48" />
        {['all','beginner','intermediate','advanced'].map(l=>(
          <button key={l} onClick={()=>setLevelFilter(l)} className={`px-4 py-2 rounded-xl text-sm transition-colors ${levelFilter===l?'bg-amber-500 text-ocean-950 font-bold':'bg-ocean-800 text-slate-400 border border-ocean-600'}`}>
      ...
      {loading ? <div className="text-center py-20 text-slate-400">در حال بارگذاری...</div> : (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4">
            ...
            return (
              <div key={b.id} className="bg-ocean-800 border border-ocean-600 rounded-xl overflow-hidden hover:border-ocean-500 transition-all">
                <div className="h-32 relative" style={{background: b.cover_url?undefined:`${color}22`}}>
                  ...
                  {!b.is_active && <span className="absolute top-2 left-2 text-xs px-1.5 py-0.5 rounded-full bg-red-900/80 text-red-300">غیرفعال</span>}
                </div>
                <div className="p-3">
          ...
          {filtered.length===0 && <div className="col-span-full text-center py-12 text-slate-500">کتابی پیدا نشد</div>}
        </div>
      )}
      {modal && <BookModal book={modal==='new'?undefined:modal as Book} cats={cats} onClose={()=>setModal(null)} onSaved={load} />}
    </div>
  )
}
